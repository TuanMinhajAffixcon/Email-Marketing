...
# df = pd.read_csv('Real_Estate_Data_20240219.csv', encoding='latin-1').fillna("")
# df['Concatenated'] = df[['Interests', 'Brand_Visited', 'Place_categories','Geo_Behaviour']].apply(lambda row: ' '.join(row), axis=1).str.strip()
# documents = df.to_dict(orient='records')

# st.write('dictioanry loaded')

# elasticsearch_url = os.getenv('ELASTICSEARCH_URL')
# elasticsearch_api_key = os.getenv('ELASTICSEARCH_API_KEY')
# es = Elasticsearch(elasticsearch_url, api_key=elasticsearch_api_key,timeout=10000)
# # es.indices.create(index="real_estate")
